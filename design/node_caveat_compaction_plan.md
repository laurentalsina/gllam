# Node Caveat Compaction & Salience Windowing Architecture Plan
...
This feature introduces **Node Caveat Compaction & Salience Windowing**, ranking active caveats while synthesizing historical caveat bloat into a node-level summary.
...
flowchart TD
    ...
    Summarizer --> CompactedSummary[Compacted Node Caveat Epoch Summary]
...
### 2. Core Engine APIs
* **`CompactNodeCaveats(ctx context.Context, nodeID string, maxInline int) (string, int, int, error)`:**
...
* **`BatchCompactHubCaveats(ctx context.Context, caveatThreshold int, maxInline int) (int, error)`:**
  * Scans for enterprise hub nodes with $> \text{caveatThreshold}$ total caveats and executes compaction.
